    ...
    */
    ...
    function isPolishScheduled(item) {
        if (!item || typeof item !== "object") {
            qtest_results.fail("Argument must be a valid Item; actual type is " + typeof item, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
    ...
    function waitForItemPolished(item, timeout) {
        if (!item || typeof item !== "object") {
            qtest_results.fail("First argument must be a valid Item; actual type is " + typeof item, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (timeout !== undefined && typeof (timeout) != "number") {
            qtest_results.fail("Second argument must be a number; actual type is " + typeof timeout, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (!timeout)
            timeout = 5000;
    ...
    function createTemporaryQmlObject(qml, parent, filePath) {
        if (typeof qml !== "string") {
            qtest_results.fail("First argument must be a string of QML; actual type is " + typeof qml, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (!parent || typeof parent !== "object") {
            qtest_results.fail("Second argument must be a valid parent object; actual type is " + typeof parent, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (filePath !== undefined && typeof filePath !== "string") {
            qtest_results.fail("Third argument must be a file path string; actual type is " + typeof filePath, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
    ...
    function createTemporaryObject(component, parent, properties) {
        if (typeof component !== "object") {
            qtest_results.fail("First argument must be a Component; actual type is " + typeof component, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (properties && typeof properties !== "object") {
            qtest_results.fail("Third argument must be an object; actual type is " + typeof properties, util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
    ...
    function qtest_compareInternal(act, exp) {
        ...
        if (act === exp) {
            ...
            if (typeExp !== typeAct)
                ...
                if ((typeExp === "string" && (typeAct === "object") || typeAct == "declarativeitem") || ((typeExp === "object" || typeExp == "declarativeitem") && typeAct === "string"))
                    success = (act == exp);

            else if (typeExp === "string" || typeExp === "boolean" || typeExp === "null" || typeExp === "undefined")
                ...
                success = (Math.abs(act.x - exp.x) <= 1e-05 && Math.abs(act.y - exp.y) <= 1e-05 && Math.abs(act.z - exp.z) <= 1e-05);
                ...
                success = (act.source === exp.source && act.global === exp.global && act.ignoreCase === exp.ignoreCase && act.multiline === exp.multiline);
    ...
    function qtest_compareInternalObjects(act, exp) {
        ...
        if (aProperties.length == 0 && bProperties.length == 0)
            // at least a special case for QUrl
    ...
    function tryCompare(obj, prop, value, timeout, msg) {
        if (arguments.length == 1 || (typeof (prop) != "string" && typeof (prop) != "number")) {
            qtest_results.fail("A property name as string or index is required for tryCompare", util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (arguments.length == 2) {
            qtest_results.fail("A value is required for tryCompare", util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (timeout !== undefined && typeof (timeout) != "number") {
            qtest_results.fail("timeout should be a number", util.callerFile(), util.callerLine());
            throw new Error("QtQuickTest::fail");
        }
        if (!timeout)
            timeout = 5000;
...
*/
    ...
    function qtest_verifyItem(item, method) {
        try {
            if (!(item instanceof Item) && !(item instanceof Window)) {
    ...
    function qtest_runInternal(prop, arg) {
        try {
            ...
            if (e.message.indexOf("QtQuickTest::") != 0)
                ...
                qtest_results.fail("Uncaught exception: " + e.message, e.fileName, e.lineNumber);
    ...
    function qtest_run() {
        ...
        let testsToRun = {
        }; // explicitly provided function names to run and their tags for data-driven tests
        if (qtest_results.functionsToRun.length > 0) {
            checkNames = true;
            var found = false;
            if (name.length > 0) {
                for (var index in qtest_results.functionsToRun) {
                    // empty tags mean run all rows
 ...
 // Must have something
                    ...
                    if (!haveData)
                        if (datafunc === "init_data")
                            ...
                            qtest_results.warn("no data supplied for " + prop + "() by " + datafunc + "()", util.callerFile(), util.callerLine());
        ...
        if (checkNames) {
            ...
            for (var func in testsToRun) {
                ...
                if (tags.length <= 0)
                    ...
                    for (var i in tags) missingTests.push(caseFuncName + ":" + tags[i]);
            }
            missingTests.sort();
    ...
    //! \internal
    TestResult {
        id: qtest_results_normal
    }

    TestEvent {
        id: qtest_events_normal
    }
    //! \internal
