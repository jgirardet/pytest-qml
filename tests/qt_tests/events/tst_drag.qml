...
//    }
    ...
    TestCase {
        ...
        function test_mouseDrag() {
            mouseDrag(container, 10, 10, qmlbot.dragThreshold * 2, qmlbot.dragThreshold * 3);
            compare(container.x, qmlbot.dragThreshold - 1);
            compare(container.y, qmlbot.dragThreshold * 2 - 1);

        }

        function test_doSomethingWhileDragging() {
            container2.updatePositionWhileDragging = false
            // dx and dy are superior to 3 times qmlbot.dragThreshold.
            // but here the dragging does not update posX and posY
            // posX and posY are only updated on mouseRelease
            container2.reset()
            mouseDrag(container2, container2.x + 10, container2.y + 10, 10*qmlbot.dragThreshold, 10*qmlbot.dragThreshold);
            ...
            // dx and dy are inferior to 3 times qmlbot.dragThreshold.
            // No intermediate mouseMove when dragging, only one mouseMove
            container2.reset()
            mouseDrag(container2, container2.x + 10, container2.y + 10, 2*qmlbot.dragThreshold, 2*qmlbot.dragThreshold);
            compare(spyX.count, 1)
            compare(spyY.count, 1)

            // dx is superior to 3 times qmlbot.dragThreshold.
            // 3 intermediate mouseMove when dragging on x axis
            // no move on the y axis
            container2.reset()
            mouseDrag(container2, container2.x + 10, container2.y + 10, 10*qmlbot.dragThreshold, 0);
            compare(spyX.count, 3)
            compare(spyY.count, 0)

            // dy is inferior to 3 times qmlbot.dragThreshold.
            // No intermediate mouseMove when dragging, only one mouseMove on y axis
            // no move on the x axis
            container2.reset()
            mouseDrag(container2, container2.x + 10, container2.y + 10, 0, 2*qmlbot.dragThreshold);
